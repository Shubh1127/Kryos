...
interface AuthContextType {
  ...
  logout: () => void;
  // Allow setting company directly after OTP-based login flow
  setCompanyFromOtp: (company: Company) => void;
}
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const register = async (companyData: {
    ...
    setIsLoading(true);
    try {
      await apiService.registerCompany(companyData);
  ...
  const value: AuthContextType = {
    ...
    register,
    logout,
    setCompanyFromOtp: (c: Company) => setCompany(c),
  };
